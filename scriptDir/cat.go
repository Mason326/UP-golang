...
Параметры:
  -A, --show-all        показать все непечатаемые символы
  -b, --number-nonblank нумеровать только непустые строки
  -E, --show-ends       выводить $ в конце каждой строки
...
Примеры:
  ...
  cat file.txt | grep "text"      Использовать в пайпе
  cat > newfile.txt               Создать новый файл (Ctrl+D для завершения)`
